...
public class EditGestureDrawLayout extends FrameLayout implements View.OnTouchListener {
    ...
    private Map<LState, View> stateToViewMap = new HashMap<>(); //通过lstate寻找对应的view，以确定连线
    ...
    public EditGestureDrawLayout(Context c, OneGestureArea model) {
        ...
        while (!(currCol = linkedStates.get(linkedStates.size()-1)).isEmpty()) {
            ...
            for(LState preState : currCol) {
                ...
                for(int eventCode : preState.events) {
                    ...
                    for(List<LState> susPostCol: linkedStates)
                        for(LState susLPostState : susPostCol)
                            if (susLPostState.state == postState) {
                                lPostState = susLPostState;
                                //如果postState所在列比pre靠左，第二遍循环考虑将其后移
                                int preColIdx = linkedStates.indexOf(currCol);
                                int postColIdx = linkedStates.indexOf(susPostCol);
                                if(postColIdx <= preColIdx) {
                                    //如果已经有更靠后的preState，则不新增，用更靠后的那个就行了
                                    boolean add = true;
                                    for(Adjustable adj: adjustableList)
                                        if(adj.postState == lPostState && adj.preColIdx >= preColIdx) {
                                            add = false;
                                            break;
                                        }
                                    if(add) {
        ...
        LinearLayout linearAllCols = new LinearLayout(c);
        linearAllCols.setOrientation(LinearLayout.HORIZONTAL);
        addView(linearAllCols);
        for(int colIdx=0; colIdx<linkedStates.size(); colIdx++) {
            ...
            LinearLayout linearCol = new LinearLayout(c);
            ...
            for(int rowIdx=0; rowIdx<col.size(); rowIdx++) {
                ...
                linearState.setOrientation(LinearLayout.VERTICAL);
                linearState.setBackgroundColor(colorStateBg);

                TextView tvState = QH.TV.one(c).text16Sp().text(lstate.state.getNiceName()).to();
                tvState.setTextColor(colorText);
                tvState.setMaxLines(3);
                tvState.setEllipsize(TextUtils.TruncateAt.END);
                linearState.addView(tvState, QH.LPLinear.one(-1,-2).left().right().top().to());


                for(int evtIdx=0; evtIdx<lstate.events.length; evtIdx++) {
                    int evt = lstate.events[evtIdx];
                    TextView tvEvt = QH.TV.one(c).text14Sp().text(FSMR.getEventS(evt)).to();
                    tvEvt.setTextColor(colorText);
                    tvEvt.setMaxLines(2);
                    tvEvt.setGravity(Gravity.RIGHT);
                    tvEvt.setEllipsize(TextUtils.TruncateAt.END);
                    linearState.addView(tvEvt, QH.LPLinear.one(widthEventText, heightEventText).gravity(Gravity.RIGHT).right().top(evtIdx==0 ? marginEventTopToTitleBottom : dp8).to());
                }


                linearCol.addView(linearState, QH.LPLinear.one(widthState, -2).top(marginStateVertical).to());

                frameTop = frameTop + lstate.pos.height()+ marginStateVertical;
            }

            linearAllCols.addView(linearCol, QH.LPLinear.one(widthState, -2).margin(dp8, dp8,
                    marginStateRightToActionLeft + widthAction + marginActionRightToNextStateLeft, dp8).to());
...
//        setLayoutParams(new ViewGroup.LayoutParams(maxWidth, maxHeight));
    ...
    private int widthState = dp8*20; //state框的宽度
    ...
    private int heightStateTitle = dp8*6; //state框内 标题（名字）的高度 (一行16dp,最多3行）（不对，没算进去行间距）
    ...
    private int heightEventText = dp8 * 8 / 2; //event和其对应的的文字高度（一行14dp，最多2行）（不对，没算进去行间距）
